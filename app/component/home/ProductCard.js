...
const ProductCard = ({ product }) => {
  ...
  const handleAddToCart = async (e) => {
    ...
    await addToCart(String(product._id), 1); // مهم: فقط آیدی و تعداد
  };
  ...
  return (
    <motion.div
      ...
      whileHover={{ boxShadow: "0 10px 30px rgba(0,0,0,0.1)", y: -5, transition: { duration: 0.3 } }}
    ...
    >
      <div className="relative overflow-hidden product-card-image-container">
        ...
        <div className="absolute bottom-2 left-2">
          <div
            className={`text-xs font-semibold px-2 py-1 rounded-lg flex items-center gap-1 shadow-sm ${
        ...
        <div className="absolute bottom-2 right-2">
          <div className="bg-white bg-opacity-90 flex items-center gap-1 px-2 py-1 rounded-lg text-xs text-gray-600 shadow-sm">
            ...
            <span>{Number(product?.views || 0).toLocaleString()}</span>
          </div>
        </div>

        <div
          className={`absolute w-full py-2 px-3 flex justify-center gap-2 transition-all duration-300 bg-gradient-to-t from-black/70 to-transparent ${
        ...
        >
          <motion.button
            ...
            className={`p-2 rounded-full shadow-lg bg-white hover:bg-blue-500 hover:text-white transition-all duration-300 ${
      ...
      <div className="p-4 flex-1 flex flex-col">
        <Link href={`/component/products/${product?._id}`} className="block text-center mb-3">
        ...
        {product?.category && typeof product.category === "object" && (
          <div className="mb-3 text-center">
            <Link href={`/category/${product.category.slug}`} className="text-xs inline-flex items-center gap-1 text-gray-500 hover:text-blue-500 transition-colors">
              <FaTags size={10} />
        ...
        <div className="mt-auto">
          <div className="flex flex-col items-center gap-1">
            ...
            <div className="flex items-center justify-center gap-1">
              <span className={`font-bold text-lg ${Number(product?.discount) > 0 ? "text-green-600" : "text-gray-800"}`}>
          ...
          <button
            onClick={handleAddToCart}
            disabled={!hasStock}
            className={`w-full mt-3 py-2 rounded-lg transition-all duration-300 font-medium text-sm flex items-center justify-center gap-2 ${
              hasStock ? "bg-blue-500 hover:bg-blue-600 text-white" : "bg-gray-200 text-gray-500 cursor-not-allowed"
            }`}
          >
            <FaCartPlus />

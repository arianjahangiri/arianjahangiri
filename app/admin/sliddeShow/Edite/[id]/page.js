...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    ...
    const fetchImageSlideShow = async () => {
      setLoading(true);
      try {
        const slidResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/SlideShow/${id}`);
        if (!slidResponse.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/SlideShow/${id}`, {
        method: "PUT",
        body: formData,
      });

...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    ...
    const fetchImageSlideShow = async () => {
      setLoading(true);
      try {
        const slidResponse = await fetch(`https://arianjahangiri.vercel.app/api/SlideShow/${id}`);
        if (!slidResponse.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://arianjahangiri.vercel.app/api/SlideShow/${id}`, {
        method: "PUT",
        body: formData,
      });

...
const UpdateAds = () => {
  ...
  useEffect(() => {
    ...
    const fetchImageAds = async () => {
      setLoading(true);
      try {
        const AdsResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/Brandsection/${id}`);
        if (!AdsResponse.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/Brandsection/${id}`, {
        method: "PUT",
        body: formData,
      });

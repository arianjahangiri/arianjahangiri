...
const UpdateAds = () => {
  ...
  useEffect(() => {
    ...
    const fetchImageAds = async () => {
      setLoading(true);
      try {
        const AdsResponse = await fetch(`https://arianjahangiri.vercel.appi.vercel.appi.vercel.app/api/Brandsection/${id}`);
        if (!AdsResponse.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://arianjahangiri.vercel.appi.vercel.appi.vercel.app/api/Brandsection/${id}`, {
        method: "PUT",
        body: formData,
      });
